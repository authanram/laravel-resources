...
<li class="{{ $action->theme(['resources.list', 'row', 'resources.actions.index.fields.row']) }}">

    @foreach ($action->getFields() as $field)

        <div class="{{ $field->getClass($action->getAction(), $action->theme(['resources.list.column', 'resources.actions.index.fields.value'])) }}">
